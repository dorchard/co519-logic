...
Note however that in predicate logic we have to be careful
...
with some variable $x$ replaced by an object. Therefore, eliminations
...
used previously for subproofs, but these boxes are not subproofs. The rule for
...
top-left, and by the fact that they will not start with any
assumptions.
...
ends here) then we can conclude $R$ overall. Importantly, this box is
...
\noindent
The natural deduction rules for first-order logic include
all those of propositional logic plus the following: \\
...
& $\begin{array}{l}\dfrac{\forall x . P}
  {P [t/x]} \; {\forall_e} \\[2.5em]\end{array}$ \\[1.25em] \hline
$\exists$
&
\rule{0cm}{0.75cm}
